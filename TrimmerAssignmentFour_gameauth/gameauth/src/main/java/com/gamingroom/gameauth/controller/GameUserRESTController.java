...
/*
 * C. Trimmer: Adding Path annotation for gameusers resource
...
public class GameUserRESTController {
    ...
    /*
     * C. Trimmer: Add annotations for RolesAllowed
     * Specify that user role is required to 
     * get a game user by its ID
     * Uses BasicAuth service to get credential validation
     */
    @RolesAllowed({"USER"})
    @GET
    @Path("/{id}")
    ...
    /*
     * C. Trimmer: Adding RolesAllowed annotation
     * Specify that the admin role is required to create
     * a game user.
     * Uses BasicAuth to validate credentials.
     */
    @RolesAllowed({"ADMIN"})
    @POST
